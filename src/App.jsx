...
const TRAILER_WEIGHT_CAPACITY = 40000
const TRAILER_PALLET_CAPACITY = 54
...
const viewConfig = {
  orders: { singular: 'order', columns: ['PRO #', 'Origin', 'Destination', 'Stage', 'Readiness', 'Pallets', 'Weight'] },
...
function routeOrders(orders, rules, trips, terminalMap) {
  ...
  return orders.map((order) => {
    ...
    const routePath = [...new Set([current, next, final])]
    const capacity = Math.max(weight / (numberFor(rule,['trailerweightcapacity']) || TRAILER_WEIGHT_CAPACITY), pallets / (numberFor(rule,['trailerpalletcapacity']) || TRAILER_PALLET_CAPACITY), cube / (numberFor(rule,['trailercubecapacity']) || TRAILER_CUBE_CAPACITY))
...
function LinehaulDashboard({ rows, rules, trips, terminalMap, selectedTerminal, setSelectedTerminal, selectedStages, setSelectedStages, onUpload, onCompleteLeg }) {
  ...
  const lanes = Object.values(eligible.reduce((all, r) => { const key=`${r.current}-${r.next}`; const lane=all[key] || { key, current:r.current, next:r.next, count:0, weight:0, pallets:0, cube:0, oldest:0, earliest:null, orders:[], serviceRisk:false, existingTrip:null }; lane.count++; lane.weight+=numberFor(r.order,['weight','totalweight','lbs']); lane.pallets+=numberFor(r.order,['palletcount','pallets']); lane.cube+=numberFor(r.order,['cube']); lane.oldest=Math.max(lane.oldest,r.dwell); lane.serviceRisk ||= r.serviceRisk; lane.existingTrip ||= r.existingTrip; lane.orders.push(r); const mabd=dateValue(field(r.order,['mabd'])); if(mabd && (!lane.earliest || mabd<lane.earliest)) lane.earliest=mabd; all[key]=lane; return all }, {})).map(l => { const rawUtilization=Math.max(l.weight/TRAILER_WEIGHT_CAPACITY,l.pallets/TRAILER_PALLET_CAPACITY,l.cube/TRAILER_CUBE_CAPACITY); const requiredTrips=Math.max(1,Math.ceil(rawUtilization)); return {...l, rawUtilization, utilization:Math.min(rawUtilization,1), requiredTrips, action:requiredTrips>1?'CAPACITY RISK':l.serviceRisk?'SERVICE RISK':rawUtilization>=.85?'BUILD NOW':rawUtilization>=.65?'BUILD SOON':'HOLD'} }).sort((a,b)=>Number(b.serviceRisk)-Number(a.serviceRisk)||b.utilization-a.utilization)
  ...
  return <div className="trip-builder-dark">
    ...
    <section className="tb-metrics"><div className="tb-metric"><span>Suggested trips</span><strong className="blue">{lanes.reduce((sum,lane)=>sum+lane.requiredTrips,0)}</strong></div><div className="tb-metric"><span>Eligible orders</span><strong>{eligible.length}</strong></div><div className="tb-metric"><span>Lanes</span><strong>{lanes.length}</strong></div><div className="tb-metric"><span>Total weight</span><strong className="orange">{eligible.reduce((sum,r)=>sum+numberFor(r.order,['weight','totalweight','lbs']),0).toLocaleString()} lbs</strong></div></section>
    <section className="tb-builder"><div className="tb-builder-head"><div>Suggested Trips <span className="tb-badge">{lanes.reduce((sum,lane)=>sum+lane.requiredTrips,0)}</span></div><span>Capacity: 40,000 lbs · 54 pallets</span></div><div className="tb-body"><aside className="tb-lanes">{lanes.length ? lanes.map(l=><button type="button" className={`tb-lane ${selected?.key===l.key?'active':''}`} onClick={()=>setActiveLane(l.key)} key={l.key}><div><strong><i>{l.current}</i> → <em>{l.next}</em></strong><b className={l.serviceRisk?'risk':''}>{l.action}</b></div><small><span>{l.count} orders · {l.pallets} plts · {Math.round(l.weight).toLocaleString()} lbs</span><span>{l.requiredTrips > 1 ? `${l.requiredTrips} trailers required` : `${Math.round(l.utilization*100)}% full`}</span></small></button>) : <div className="tb-empty">No linehaul-eligible orders for these filters.</div>}</aside><div className="tb-detail">{selected ? <article className="tb-trip-card"><div className="tb-trip-top"><strong><i>{selected.current}</i> → <em>{selected.next}</em></strong><div><button type="button" className="tb-ghost">{selected.existingTrip ? `Co-load on ${field(selected.existingTrip,['tripnumber','id'])}` : 'Review capacity'}</button><button type="button" className="tb-primary" onClick={()=>onCompleteLeg(selected.orders)}>Complete leg & re-route</button></div></div><div className="tb-route-context">Active leg: <strong>{selected.current} → {selected.next}</strong> <span>Approved path: {selected.orders[0]?.routePath?.join(' → ') || `${selected.current} → ${selected.next}`}</span></div><p>{selected.pallets} Plts · {Math.round(selected.weight).toLocaleString()} Lbs · <b>{selected.requiredTrips > 1 ? `${selected.requiredTrips} trailers required` : `${Math.round(selected.utilization*100)}% Full`}</b>{selected.earliest ? ` · MABD ${selected.earliest.toLocaleDateString()}` : ''}</p><div className="tb-progress"><div style={{width:`${Math.min(selected.utilization*100,100)}%`}}/></div><div className="tb-table-wrap"><table><thead><tr><th>PRO</th><th>Consignee city</th><th>State</th><th>Weight</th><th>Pallets</th><th>MABD</th><th>Status</th></tr></thead><tbody>{selected.orders.sort((a,b)=>Number(b.serviceRisk)-Number(a.serviceRisk)||a.dwell-b.dwell).map((r,i)=><tr key={i}><td>{field(r.order,['pro','pronumber'])||'—'}</td><td>{field(r.order,['destinationcity','consigneecity','city'])||'—'}</td><td>{field(r.order,['destinationstate','consigneestate','state'])||'—'}</td><td>{numberFor(r.order,['weight','totalweight','lbs']).toLocaleString()} lbs</td><td>{numberFor(r.order,['palletcount','pallets'])}</td><td>{dateValue(field(r.order,['mabd']))?.toLocaleString()||'—'}</td><td className={r.serviceRisk?'risk':'status'}>{r.status}</td></tr>)}</tbody></table></div></article> : <div className="tb-empty">Select a lane to review eligible orders.</div>}</div></div></section>
  </div>
}

function QuotesWorkspace({ rows, query, setQuery, onUpload }) {
  const shipments = rows.map((order) => ({
    order: field(order,['ordernumber','order']) || '—', pro: field(order,['pronumber','pro']) || '—', customer: field(order,['customer','billto']) || '—', origin: field(order,['originterminal','currentterminal','origin']) || '—', city: field(order,['destinationcity','consigneecity']) || '—', state: field(order,['destinationstate','consigneestate']) || '—', weight: numberFor(order,['weight','totalweight','lbs']), pallets: numberFor(order,['palletcount','pallets']), stage: field(order,['currentstage','stage']) || '—',
  })).filter((row) => !query || Object.values(row).join(' ').toLowerCase().includes(query.toLowerCase()))
  return <div className="quotes-workspace"><section className="quotes-hero"><div><span>LOCAL RATE WORKSPACE</span><h2>Quotes</h2><p>Review shipment inputs from the loaded plan. Rates are not requested from external services.</p></div><button className="primary-button" type="button" onClick={onUpload}><Upload size={14}/> Upload local plan</button></section><section className="quotes-stats"><div><span>Shipment inputs</span><strong>{rows.length.toLocaleString()}</strong></div><div><span>Customer accounts</span><strong>{new Set(rows.map(order=>field(order,['customer','billto'])).filter(Boolean)).size}</strong></div><div><span>Rate source</span><strong>Local only</strong></div></section><section className="quotes-table-panel"><div className="module-toolbar"><div className="search-wrap"><Search size={14}/><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Search shipment inputs..."/></div></div><div className="quotes-table-title"><div><h3>Available shipment inputs</h3><p>Select a record after a local rate plan is loaded.</p></div><span>{shipments.length} records</span></div><div className="table-panel"><table className="data-table"><thead><tr><th>Order / PRO</th><th>Customer</th><th>Origin</th><th>Consignee</th><th>Weight</th><th>Pallets</th><th>Stage</th></tr></thead><tbody>{shipments.slice(0,200).map((row,index)=><tr key={index}><td><strong>{row.order}</strong><small>{row.pro}</small></td><td>{row.customer}</td><td>{row.origin}</td><td>{row.city}{row.state ? `, ${row.state}` : ''}</td><td>{row.weight.toLocaleString()} lbs</td><td>{row.pallets}</td><td>{row.stage}</td></tr>)}</tbody></table></div></section></div>
}

function OperationalModule({ view, config, rows, trips, query, setQuery }) {
  const [filtersOpen, setFiltersOpen] = useState(false)
  const orders = rows.map((order) => ({
    order: field(order, ['ordernumber', 'order']) || '—', pro: field(order, ['pronumber', 'pro']) || '—', origin: field(order, ['currentterminal', 'originterminal', 'origin']) || '—', city: field(order, ['destinationcity', 'consigneecity']) || '—', state: field(order, ['destinationstate', 'consigneestate']) || '—', stage: field(order, ['currentstage', 'stage', 'status']) || '—', pallets: numberFor(order, ['palletcount', 'pallets']), weight: numberFor(order, ['weight', 'totalweight', 'lbs']), trip: field(order, ['assignedtrip', 'tripnumber']) || '—', customer: field(order, ['customer', 'billto']) || '—', local: String(field(order,['currentterminal','originterminal','origin'])).toUpperCase() === finalTerminal(order, {}), readiness: ['Pickup Complete','Linehaul Complete'].includes(field(order,['currentstage','stage','status'])) ? 'Ready for linehaul' : field(order,['currentstage','stage','status']) === 'Pickup' ? 'Awaiting pickup complete' : field(order,['currentstage','stage','status']) === 'Linehaul' ? 'In linehaul transit' : 'Review stage'
  }))
  ...
  if (view === 'orders') { data=orders; cells=(r)=>[r.pro,r.origin,`${r.city}, ${r.state}`,r.stage,r.readiness,r.pallets,`${r.weight.toLocaleString()} lbs`] }
...
function App() {
  ...
  function completeLeg(routedOrders) {
    const moves = new Map(routedOrders.map((result) => [String(field(result.order, ['ordernumber', 'order', 'pro', 'pronumber'])), result.next]))
    setRows((currentRows) => currentRows.map((order) => {
      const key = String(field(order, ['ordernumber', 'order', 'pro', 'pronumber']))
      const nextTerminal = moves.get(key)
      return nextTerminal ? { ...order, currentTerminal: nextTerminal, currentStage: 'Linehaul Complete', assignedTrip: '', tripStatus: '' } : order
    }))
    setNotice({ type: 'success', text: `${moves.size} order${moves.size === 1 ? '' : 's'} completed locally and re-routed from the new current terminal` })
  }
  ...
  if (activeView === 'linehaul') {
    content = <LinehaulDashboard rows={rows} rules={rules} trips={trips} terminalMap={terminalMap} selectedTerminal={selectedTerminal} setSelectedTerminal={setSelectedTerminal} selectedStages={selectedStages} setSelectedStages={setSelectedStages} onUpload={() => fileInput.current?.click()} onCompleteLeg={completeLeg} />
  } else if (activeView === 'quotes') {
    content = <QuotesWorkspace rows={rows} query={query} setQuery={setQuery} onUpload={() => fileInput.current?.click()} />
